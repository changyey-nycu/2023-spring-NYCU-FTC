...
public:
  ...
  int Auth(std::string challenge)
  {
    ...
    file.read(sktmp, crypto_sign_SECRETKEYBYTES);
    file.close();

    // detached signature
    unsigned char *sk = (unsigned char *)sktmp;
    ...
    {
      token = authtoken;
      std::cout << "Auth successed token:" << authtoken.value() << std::endl;
      return 0;
    }
    return 1;
  }
